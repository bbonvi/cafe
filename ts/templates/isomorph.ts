import { blurPath } from './../posts/images';
...
export function makePostContext(
  ...
  ctx.Files = (p.files || []).map((img: ImageData, n: number) =>
    new TemplateContext("post-file", {
      ...
      BlurPath: blurPath(img.thumbType, img.SHA1),
      ThumbPath: thumbPath(img.thumbType, img.SHA1),
      ThumbPng: img.thumbType === 1 ? true : false
    }).render(),
  )
...
export function duration(l: number, cut?: boolean): string {
  if (!l) return '0:00'
  if (cut) return Math.floor(l / 60) + ":" + pad(Math.floor(l % 60))
...
export function fileSize(size: number): string {
  if (size < 1024) {
    return Math.round(size) + _("b")
  } else if (size < 1024 * 1024) {
    return Math.round(size / 1024) + _("kb")
  } else {
    return (size / 1024 / 1024).toFixed(1) + _("mb")
  }
}
...
function ago(msgid1: string, msgid2: string, time: number, isFuture: boolean): string {
  ...
  return isFuture ? printf(_("in %s %s"), time, unit) : printf(_("%s %s ago"), time, unit)
}
