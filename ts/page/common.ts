...
import { mine, page, posts, storeSmiles } from "../state";
...
/**
 * Need to manually specify board when on admin page.
 */
export async function updateBoardSmiles(board?: string) {
    const { threads } = extractPageData<ThreadData>();
    if (board || threads.board) {
        const boardSmiles = await API.smiles.get(board || threads.board);
        storeSmiles(boardSmiles, (board || threads.board));
    }
}

export async function updateGlobalSmiles() {
    return updateBoardSmiles("all");
}
