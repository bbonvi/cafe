...
func fileSize(l string, size int) string {
	...
	case size < 1024*1024:
		return fmt.Sprintf("%d%s", int32(size)/1024, lang.Get(l, "kb"))
	default:
		return fmt.Sprintf("%.1f%s", float32(size)/1024/1024, lang.Get(l, "mb"))
	}
}
...
func (ctx *PostContext) renderFile(img *common.Image, n int) string {
	...
	fileCtx := FileContext{
		...
		BlurPath:   assets.BlurPath(img.ThumbType, img.SHA1),
		ThumbPng:   isPngThumb,
	}
