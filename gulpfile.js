...
// TODO: THIS IS REALLY STUPID WAY OF DOING IT!!
gulp.task("smiles-gif", () => {
    const smileNames = fs.readdirSync("smiles");
    const smileIds = smileNames
      .filter(n => /^[a-z0-9_]+\.gif/.test(n))
      .map(n => n.slice(0, -4))
      .sort();
    smileIds.forEach(id => {
      fs.copyFileSync(`smiles/${id}.gif`, `assets/img/smiles-${id}.gif`);
      const css = `
          .smile-${id} {
              background-image: url(/static/img/smiles-${id}.gif)!important;
      ...
      fs.appendFile('smiles-pp/smiles.css', css, (err) => {
        if (err) throw err;
      });
    })
})
...
gulp.task("smiles", () => {
    ...
    let hash = "";
    const smileNames = fs.readdirSync("smiles").filter(n => /.png$/.test(n))
    const smileIds = smileNames
      .filter(n => /^[a-z0-9_]+\.png$/.test(n))
      .map(n => n.slice(0, -4))
      .sort();

    const smileIdsGif = fs.readdirSync("smiles")
      .filter(n => /^[a-z0-9_]+\.gif/.test(n))
      .map(n => n.slice(0, -4))
      .sort();
    ...
    return gulp.src(smilePaths)
      ...
      .on("end", () => {
        const jsSmiles = [...smileIds, ...smileIdsGif].map(id => `"${id}"`).join(",");
        const jsModule = `
        ...
        const goSmiles = [...smileIds, ...smileIdsGif].map(id => `"${id}":true`).join(",");
        const goModule = `
          // AUTOGENERATED, DO NOT EDIT
          package smiles
        ...
        runSequence("smiles-gif", "assets", "css", "es6")
      });
  });
